...
class Neo4jMoviesDb(MoviesDb):
    ...
    def __init__(self,movies_config,movies_logger):
        ...
        try:
            ...
            self._movies_logger.LogError("Problem occured while connecting to neo4j DB and initializing.Exception:{0}".format(e))
    ...
    def insert(self,movie):
        if(self.isExists(movie) is False):
            ...
            try:
                ...
                self._movies_logger.LogError("Failed creating node Person for -{0}.Exception:{1}".format(movie.director, e))
            ...
            try:
                ...
                self._movies_logger.LogError(
                    "Failed creating relationship between director -{0} and Movie - {1}.Exception:{2}".format(movie.director,movie.name, e))
            ...
            for actor in movie.actors:
                ...
                try:
                    ...
                    self._movies_logger.LogError(
                        "Failed creating relationship between actor -{0} and Movie - {1}.Exception:{2}".format(actor,movie.name, e))
